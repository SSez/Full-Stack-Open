...
const PatientPage = () => {
  ...
  const submitNewEntry = async (values: EntryFormValues) => {
    ...
    const { data: patientFromApi } = await axios.post<Patient>(`${apiBaseUrl}/patients/${id}/entries`, values);
    dispatch(setPatient(patientFromApi));
    closeModal();
  };
  ...
  const genderIcon = patient.gender === 'female' ? 'venus' : (patient.gender === 'male' ? 'mars' : 'genderless');
